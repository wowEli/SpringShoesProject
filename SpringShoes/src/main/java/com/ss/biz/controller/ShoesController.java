...
//		for(int i =0; i < sVO.getFilterSize().length; i++) {
//			System.out.println("필터 size들"+sVO.getFilterSize()[i]);			
//		}
		
		
		
		if(sVO.getFilterBrand() == null) {
			sVO.setFilterBrand("");
		}
		if(sVO.getFilterColor() == null) {
			sVO.setFilterColor("");
		}
		
		// 페이징처리에 사용할 변수들
		model.addAttribute("pageFilterSizes", sVO.getFilterSize());
		model.addAttribute("pageFilterBrand", sVO.getFilterBrand());
		model.addAttribute("pageFilterColor", sVO.getFilterColor());
